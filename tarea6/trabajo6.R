...
# Unir los datos originales y transformados en un solo data frame
datos_combined <- datos_largos %>%
  gather(key = "Tipo", value = "N2O_values", N2O, transform) %>%
  mutate(Tipo = factor(Tipo, levels = c("N2O", "transform"), 
                       labels = c("Antes de la Transformación", "Después de la Transformación")))

# Crear el Boxplot
ggplot(datos_combined, aes(x = Emisores, y = N2O_values, fill = Tipo)) +
  geom_boxplot() +
  facet_wrap(~Tipo) +
  labs(title = "Boxplot de N2O antes y después de la Transformación",
       x = "Emisores", y = "Emisiones de N₂O") +
  theme_minimal() +
